...
sub SendData{
    ...
    WriteFile "$WebDir/$DATE-sentdata.txt", $sentdata;
...
while (scalar(@ARGV) and ($_ = $ARGV[0], /^[-+]/)) {
  ...
  # List command line options here...
    ...
    if (/^-nickname/)  {
	$nickname = "$ARGV[0]"; shift; next;
    }
    #if (/^-gnuplotscript$/)  { $PlotScriptFilename = $ARGV[0]; shift; next; } ~PJ on upgrade
    #if (/^-templatefile$/)   { $Template = $ARGV[0]; shift; next; } ~PJ on upgrade
    if (/^-gccpath/)         {
...
#$LOC = `utils/countloc.sh`; ~PJ on upgrade
...
my $host = "17.201.27.159";
my $file = "/cgi-bin/NightlyTestAccept.cgi";
