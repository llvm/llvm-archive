...
build_function_call (tree function, tree params)
{
  ...
  if (inside_iasm_block
	...
	error ("%<,%> not allowed in asm");
      if (TREE_VALUE (params) == NULL_TREE)
	error ("exepcted operand inside %<()%>");
      return iasm_build_register_offset (function, TREE_VALUE (params));
    }
  /* APPLE LOCAL end CW asm blocks */
  ...
  /* Conversions among pointers */
	      ...
	      == c_common_unsigned_type (mvr)))
	{
	  ...
	  /* Const and volatile mean something different for function types,
		...
		{
		  /* APPLE LOCAL begin radar 4193359 */
		  /* APPLE LOCAL -Wdiscard-qual 4086969 */
