...
Function *TreeToLLVM::FinishFunctionBody() {
  ...
  if (Fn->getReturnType() != Type::VoidTy) {
    if (!isAggregateTreeType(TREE_TYPE(DECL_RESULT(FnDecl)))) {
      ...
      // and return it.
      RetVal = new LoadInst(DECL_LLVM(DECL_RESULT(FnDecl)), "retval", CurBB);
      RetVal = CastToType(RetVal, Fn->getReturnType());
    } else {
      ...
      // loading.
      RetVal = NOOPCastToType(DECL_LLVM(DECL_RESULT(FnDecl)),
...
bool TreeToLLVM::isNoopCast(Value *V, const Type *Ty) {
  ...
  if (isa<PointerType>(VTy)) {
    return Ty->isInteger() && Ty->getPrimitiveSizeInBits() == POINTER_SIZE;
  } else if (isa<PointerType>(Ty)) {
    return VTy->isInteger() && VTy->getPrimitiveSizeInBits() == POINTER_SIZE;
  }
...
void TreeToLLVM::EmitAggregateCopy(Value *DestPtr, Value *SrcPtr, tree type,
  ...
  if (TREE_CODE(TYPE_SIZE(type)) == INTEGER_CST &&
    ...
    if (CountAggregateElements(LLVMTy) <= 8) {
      DestPtr = CastToType(DestPtr, PointerType::get(LLVMTy));
      SrcPtr = CastToType(SrcPtr, PointerType::get(LLVMTy));
...
void TreeToLLVM::EmitAutomaticVariableDecl(tree decl) {
  ...
  if (DECL_SIZE(decl) == 0) {    // Variable with incomplete type.
    ...
    if (TREE_CODE(type) == ARRAY_TYPE && TYPE_DOMAIN(type)) {
      ...
      Size = CastToType(Size, Type::UIntTy);
      ...
      Size = BinaryOperator::createAdd(Size, ConstantInt::get(Type::UIntTy, 1),
                                       "tmp", CurBB);
    } else {
      // Compute the variable's size in bytes.
      Size = CastToType(Emit(DECL_SIZE_UNIT(decl), 0), Type::UIntTy);
      Ty = Type::SByteTy;
    }
  }
...
Value *TreeToLLVM::EmitGOTO_EXPR(tree exp) {
  if (TREE_CODE(TREE_OPERAND(exp, 0)) == LABEL_DECL) {
    ...
    V = CastToType(V, TD.getIntPtrType());
...
Value *TreeToLLVM::EmitSWITCH_EXPR(tree exp) {
  ...
  for (unsigned i = 0, e = TREE_VEC_LENGTH(Cases); i != e; ++i) {
    ...
    Val = CastToType(Val, SwitchExp->getType());
...
Value *TreeToLLVM::EmitLoadOfLValue(tree exp, Value *DestLoc) {
  ...
  if (!LV.isBitfield()) {
    ...
    if (!DestLoc) {
      ...
      Value *Ptr = CastToType(LV.Ptr, PointerType::get(Ty));
    ...
    if (ValSizeInBits-LV.BitSize) {
      // If this is an unsigned/signed bitfield reference and the LLVM value is
      // signed/unsigned, cast to the appropriate signedness so that the shift 
      // coming up with do the right thing.
      if (TYPE_UNSIGNED(TREE_TYPE(exp)) != Val->getType()->isUnsigned()) {
        if (TYPE_UNSIGNED(TREE_TYPE(exp)))
          Val = CastToType(Val, Val->getType()->getUnsignedVersion());
        else
          Val = CastToType(Val, Val->getType()->getSignedVersion());
      }
        
      Value *ShAmt = ConstantUInt::get(Type::UByteTy, ValSizeInBits-LV.BitSize);
...
Value *TreeToLLVM::EmitADDR_EXPR(tree exp) {
  ...
  return CastToType(LV.Ptr, ConvertType(TREE_TYPE(exp))); 
}

Value *TreeToLLVM::EmitOBJ_TYPE_REF(tree exp) {
  return CastToType(Emit(OBJ_TYPE_REF_EXPR(exp), 0), 
...
FIXME: "Call terminate if needed!";
  ...
  for (tree arg = TREE_OPERAND(exp, 1); arg; arg = TREE_CHAIN(arg)) {
    ...
    // If we are implicitly passing the address of this thing instead of the
    // value by value, handle this first.
    if (isPassedByInvisibleReference(TREE_TYPE(TREE_VALUE(arg)))) {
      ...
      CallOperands.push_back(CastToType(Ptr, ArgTy));
    } else if (ActualArgTy->isFirstClassType()) {
      Value *V = Emit(TREE_VALUE(arg), 0);
      CallOperands.push_back(CastToType(V, ArgTy));
    } else {
  ...
  DestLoc = NOOPCastToType(DestLoc, PointerType::get(Call->getType()));
...
Value *TreeToLLVM::EmitMODIFY_EXPR(tree exp, Value *DestLoc) {
  ...
  if (isGCC_SSA_Temporary(TREE_OPERAND(exp, 0))) {
    ...
    RHS = CastToType(RHS, ConvertType(TREE_TYPE(TREE_OPERAND(exp, 0))));
...
Value *TreeToLLVM::EmitPtrBinOp(tree exp, unsigned Opc) {
  ...
  LHS = CastToType(LHS, UIntPtrTy);
  RHS = CastToType(RHS, UIntPtrTy);
...
Value *TreeToLLVM::EmitTruthOp(tree exp, unsigned Opc) {
  ...
  // This is a truth operation like the strict &&,||,^^.  Convert to bool.
  LHS = CastToType(LHS, Type::BoolTy);
  RHS = CastToType(RHS, Type::BoolTy);
  
  Value *Res = BinaryOperator::create((Instruction::BinaryOps)Opc, LHS, RHS,
                                      "tmp", CurBB);
  return CastToType(Res, ConvertType(TREE_TYPE(exp)));
}
...
Value *TreeToLLVM::EmitShiftOp(tree exp, Value *DestLoc, unsigned Opc) {
  ...
  Value *LHS = Emit(TREE_OPERAND(exp, 0), 0);
  Value *RHS = Emit(TREE_OPERAND(exp, 1), 0);
  RHS = CastToType(RHS, Type::UByteTy);
  
  if (TYPE_UNSIGNED(TREE_TYPE(exp)) != LHS->getType()->isUnsigned()) {
    if (TYPE_UNSIGNED(TREE_TYPE(exp)))
      LHS = CastToType(LHS, LHS->getType()->getUnsignedVersion());
    else
      LHS = CastToType(LHS, LHS->getType()->getSignedVersion());
  }
  
  return new ShiftInst((Instruction::OtherOps)Opc, LHS, RHS,
                       "tmp", CurBB);
}

Value *TreeToLLVM::EmitRotateOp(tree exp, unsigned Opc1, unsigned Opc2) {
  Value *In  = Emit(TREE_OPERAND(exp, 0), 0);
  Value *Amt = CastToType(Emit(TREE_OPERAND(exp, 1), 0), Type::UByteTy);
  
  // Make sure that the value is an unsigned type, so that we do a logical shr.
  In = CastToType(In, In->getType()->getUnsignedVersion());

  Value *TypeSize =
    ConstantUInt::get(Type::UByteTy, In->getType()->getPrimitiveSizeInBits());
...
#define HANDLE_UNARY_FP(F32, F64, V) \
  ...
  case BUILT_IN_SQRTL:
    ...
    if (!flag_errno_math) {
      ...
      Result = CastToType(Result, ConvertType(TREE_TYPE(exp)));
      return true; 
    }
    break;
...
bool TreeToLLVM::EmitBuiltinUnaryIntOp(Value *InVal, Value *&Result,
  ...
  // Integer Intrinsics only come in one version for each integer size, and we
  // have standardized on unsigned operands.  Convert the operand to unsigned
  // if it is signed.
  if (InVal->getType()->isSigned())
    InVal = CastToType(InVal, InVal->getType()->getUnsignedVersion());
  ...
  Result = CastToType(Result, Type::IntTy);
...
bool TreeToLLVM::EmitBuiltinExtendPointer(tree exp, Value *&Result) {
  ...
  Result = CastToType(Amt, ConvertType(TREE_TYPE(exp)));
  return true;
}
...
bool TreeToLLVM::EmitBuiltinBZero(tree_node *exp, Value *&Result) {
  ...
  Value *DstV = Emit(Dst, 0);
  Value *Val = Constant::getNullValue(Type::IntTy);
...
bool TreeToLLVM::EmitBuiltinPrefetch(tree exp) {
  ...
  if (TREE_CHAIN(arglist)) { // Args 1/2 are optional
    ...
    if (!isa<ConstantInt>(ReadWrite)) {
      ...
      ReadWrite = 0;
    } else if (cast<ConstantInt>(ReadWrite)->getRawValue() > 1) {
    ...
    if (TREE_CHAIN(TREE_CHAIN(arglist))) {
      ...
      if (!isa<ConstantInt>(Locality)) {
        ...
        Locality = 0;
      } else if (cast<ConstantInt>(Locality)->getRawValue() > 3) {
  ...
  if (ReadWrite == 0)
    ReadWrite = Constant::getNullValue(Type::UIntTy);
  if (Locality == 0)
    Locality = ConstantInt::get(Type::UIntTy, 3);
  
  Ptr = CastToType(Ptr, PointerType::get(Type::SByteTy));
  
  static Function *llvm_prefetch_fn = 0;
  if (!llvm_prefetch_fn)
    llvm_prefetch_fn = 
      TheModule->getOrInsertFunction("llvm.prefetch", Type::VoidTy,
                                     Ptr->getType(), Type::UIntTy, Type::UIntTy,
                                     NULL);
...
bool TreeToLLVM::EmitBuiltinReturnAddr(tree exp, Value *&Result, bool isFrame) {
  ...
  Result = CastToType(Result, TREE_TYPE(exp));
  return true;
}
...
bool TreeToLLVM::EmitBuiltinStackSave(tree exp, Value *&Result) {
  ...
  if (!Fn)
    Fn = TheModule->getOrInsertFunction("llvm.stacksave",
                                       PointerType::get(Type::SByteTy), NULL);
...
bool TreeToLLVM::EmitBuiltinAlloca(tree exp, Value *&Result) {
  ...
  if (!validate_arglist(arglist, INTEGER_TYPE, VOID_TYPE))
    return false;
  Value *Amt = Emit(TREE_VALUE(arglist), 0);
  Amt = CastToType(Amt, Type::UIntTy);
  Result = new AllocaInst(Type::SByteTy, Amt, "tmp", CurBB);
  return true;
}
...
bool TreeToLLVM::EmitBuiltinVAStart(tree exp) {
  ...
  ArgVal = CastToType(ArgVal, cast<FunctionType>(FTy)->getParamType(0));
...
bool TreeToLLVM::EmitBuiltinVAEnd(tree exp) {
  ...
  Arg = CastToType(Arg, cast<FunctionType>(FTy)->getParamType(0));
...
bool TreeToLLVM::EmitBuiltinVACopy(tree exp) {
  ...
  Arg1 = CastToType(Arg1, PtrList);
  Arg2 = CastToType(Arg2, PtrList);
...
LValue TreeToLLVM::EmitLV_ARRAY_REF(tree exp) {
  ...
  if (IndexVal->getType() != Type::IntTy &&
  ...
  if (TREE_CODE (TREE_TYPE(Array)) == ARRAY_TYPE) {
    return new GetElementPtrInst(ArrayAddr, ConstantInt::get(Type::IntTy, 0),
                                 IndexVal, "tmp", CurBB);
  } else {
    return new GetElementPtrInst(ArrayAddr, IndexVal, "tmp", CurBB);
  }
...
LValue TreeToLLVM::EmitLV_COMPONENT_REF(tree exp) {
  ...
  StructAddrLV.Ptr = CastToType(StructAddrLV.Ptr, PointerType::get(StructTy));
  ...
  if (TREE_CODE(DECL_FIELD_OFFSET(FieldDecl)) == INTEGER_CST) {
    ...
    if (CI->getValue() == ~0U) {
      ...
      CI = ConstantUInt::get(Type::UIntTy, StructTy->getNumContainedTypes()-1);
    }
    ...
    if (CI->getValue()) {
      ...
      assert(CI->getValue() < SL->MemberOffsets.size() &&"Idx out of range!");
      BitStart -= SL->MemberOffsets[CI->getValue()] * 8;
    }
    
  } else {
    Value *Offset = Emit(DECL_FIELD_OFFSET(FieldDecl), 0);
    Value *Ptr = CastToType(StructAddrLV.Ptr, Offset->getType());
    Ptr = BinaryOperator::createAdd(Ptr, Offset, "tmp", CurBB);
    FieldPtr = CastToType(Ptr, PointerType::get(FieldTy));
  }
  ...
  if (tree DeclaredType = DECL_BIT_FIELD_TYPE(FieldDecl)) {
    ...
    if (DECL_SIZE(FieldDecl) && TREE_CODE(DECL_SIZE(FieldDecl)) == INTEGER_CST){
      ...
      if (BitStart+BitfieldSize > LLVMValueBitSize) {
        ...
        Constant *Offset = ConstantUInt::get(TD.getIntPtrType(), ByteOffset);
...
LValue TreeToLLVM::EmitLV_BIT_FIELD_REF(tree exp) {
  ...
  if (unsigned UnitOffset = BitStart / ValueSizeInBits) {
    ...
    Ptr.Ptr = new GetElementPtrInst(Ptr.Ptr,
                                    ConstantUInt::get(Type::UIntTy, UnitOffset),
                                    "tmp", CurBB);
  ...
  if (BitStart == 0 && BitSize == ValueSizeInBits)
    return LValue(NOOPCastToType(Ptr.Ptr, PointerType::get(ValTy)));
...
Value *TreeToLLVM::EmitCONSTRUCTOR(tree exp, Value *DestLoc) {
  ...
  case UNION_TYPE:
    ...
    if (!ConvertType(TREE_TYPE(TREE_PURPOSE(elt)))->isFirstClassType()) {
      ...
      DestLoc = CastToType(DestLoc, PointerType::get(V->getType()));
...
Constant *TreeConstantToLLVM::ConvertINTEGER_CST(tree exp) {
  ...
  const Type *Ty = ConvertType(TREE_TYPE(exp));
  return ConstantExpr::getCast(ConstantUInt::get(Type::ULongTy, IntValue), Ty);
}
...
static Constant *InsertBitFieldValue(uint64_t ValToInsert,
  ...
  if (FieldTy->isInteger()) {
    ...
    if (Val == 0)
      ...
      ExistingVal = cast<ConstantInt>(Val)->getRawValue();
    }
    ...
    return ConstantExpr::getCast(ConstantUInt::get(Type::ULongTy, NewVal),
                                 FieldTy);
  } else {
    // Otherwise, this is initializing part of an array of bytes.  Recursively
    // insert each byte.
    assert(isa<ArrayType>(FieldTy) && 
           cast<ArrayType>(FieldTy)->getElementType() == Type::UByteTy &&
    ...
    if (Val) {
      if (ConstantArray *CA = dyn_cast<ConstantArray>(Val)) {
        ...
        Constant *Elt = Constant::getNullValue(Type::UByteTy);
        for (unsigned i = 0, e = Elts.size(); i != e; ++i)
          Elts[i] = Elt;
      }
    }
...
static void ProcessBitFieldInitialization(tree Field, Value *Val,
  ...
  uint64_t BitfieldVal = cast<ConstantInt>(Val)->getRawValue();
  ...
  if (TYPE_SIZE(UnionType) && TREE_CODE(TYPE_SIZE(UnionType)) == INTEGER_CST) {
    ...
    if (UnionSize != InitSize) {
      ...
      if (UnionSize - InitSize == 1)
        ...
        FillTy = ArrayType::get(Type::UByteTy, UnionSize - InitSize);
...
Constant *TreeConstantToLLVM::EmitLV_ARRAY_REF(tree exp) {
  ...
  if (TREE_CODE (TREE_TYPE(Array)) == ARRAY_TYPE)
    Idx.push_back(ConstantInt::get(Type::IntTy, 0));
